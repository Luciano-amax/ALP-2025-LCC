...
En ese caso la parte \texttt{x + 1} podría parsearse bien, pero quedaría texto sobrante. El proyecto rechaza esa entrada para evitar resultados inesperados. Esta decisión vuelve el DSL más estricto y facilita detectar errores del usuario.
...
Una computación \texttt{EvalM env a} recibe un entorno de variables y puede devolver un error o un valor. Conceptualmente combina dos ideas vistas en la materia: lectura de contexto, similar a \texttt{Reader}, y propagación de errores, similar a \texttt{Either}. Se implementó a mano para que las instancias sean visibles y para que el trabajo muestre claramente qué efecto se está modelando.
...
El proyecto también define \texttt{return = pure} en la instancia de \texttt{Monad}. Esto respeta la jerarquía actual de clases de Haskell, donde toda mónada también debe ser aplicativa. En el evaluador se usan \texttt{return} y \texttt{>>=} para que el uso monádico sea explícito, especialmente en las operaciones donde hay validaciones intermedias.
...
\section{Interfaz pública y ejecutable}

El módulo \texttt{DSL} reexporta las piezas principales del lenguaje. Su función es ofrecer una API limpia para ejemplos, tests o usos futuros. Desde ahí se puede acceder al AST, parser, evaluador, mónada de evaluación, pretty printer y procesamiento de archivos.

El ejecutable queda separado en \texttt{app/}. \texttt{Main} interpreta argumentos y decide entre modo interactivo, modo archivo o ayuda. \texttt{Console} maneja la interacción: lee una expresión, pide el valor de \texttt{x}, evalúa y muestra resultados. Esta separación evita que la lógica del lenguaje quede mezclada con entrada/salida.
...
En particular, no se busca producir una fórmula simplificada para la derivada. Si se evalúa \texttt{sin(x\^{}2)} en un punto, el sistema devuelve el valor y la derivada en ese punto, no una expresión simbólica equivalente a \(2x\cos(x^2)\). Esta limitación es deliberada y coherente con la técnica elegida.
...
Las extensiones más naturales serían permitir elegir la variable de derivación desde la entrada de usuario, ampliar el formato de archivo para entornos multivariables y mejorar la recuperación de errores del parser con mensajes más orientados a posición y contexto. También podrían agregarse propiedades de testeo más generales para parser, pretty printer y optimizador.
