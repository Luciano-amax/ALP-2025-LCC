...
evalDual (Tan x) g = do
  Dual f df <- evalDual x g   -- Evalúa recursivamente la subexpresión x
  return $ Dual (tan f) ((1 / ((cos f) ** 2)) * df)
evalDual (Exp e) x = do
  Dual p d <- evalDual e x
...
evalDual (Arcosh e) x = do
  Dual f g <- evalDual e x
  if f < 1 then Left (DomainError "Valor fuera del dominio de arcosh")
  else return $ Dual (acosh f) ((1 / sqrt (f**2 - 1)) * g)
evalDual (Artanh e) x = do
  Dual f g <- evalDual e x
  if abs f >= 1 then Left (DomainError "Valor fuera del dominio de artanh")
