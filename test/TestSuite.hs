...
-- En tests conviene comparar pares simples en vez de todo el constructor Dual.
...
case5 = TestCase $ do
  ...
  assertEqual "Parsing y evaluacion de 3 + x^2" (Right 7.0) result
...
-- Casos unitarios del lenguaje: funciones, dominios y optimizaciones.
testSqrt :: Test
testSqrt = TestList
  ...
  , TestCase $ do
      ...
      case result of
        ...
        _ -> assertFailure "sqrt(-1) deberia dar error de dominio"
  ]
...
-- Validaciones de dominio en evaluacion escalar.
...
testValidacionDominios = TestList
  [ TestCase $ do
      ...
      case result of
        ...
        _ -> assertFailure "1/0 deberia dar DivideByZero"
  , TestCase $ do
      let result = eval (Log (Lit 0)) 0
      case result of
        Left (DomainError _) -> return ()
        _ -> assertFailure "log(0) deberia dar error de dominio"
  , TestCase $ do
      let result = eval (Log (Lit (-1))) 0
      case result of
        Left (DomainError _) -> return ()
        _ -> assertFailure "log(-1) deberia dar error de dominio"
  , TestCase $ do
      let result = eval (Arcosh (Lit 0.5)) 0
      case result of
        Left (DomainError _) -> return ()
        _ -> assertFailure "arcosh(0.5) deberia dar error de dominio"
  , TestCase $ do
      let result = eval (Artanh (Lit 2)) 0
      case result of
        Left (DomainError _) -> return ()
        _ -> assertFailure "artanh(2) deberia dar error de dominio"
  , TestCase $ do
...
-- Identidades que antes eran buenas candidatas a producir NaN.
...
-- Parser y lector de archivos en casos que suelen romper precedencias.
...
testCasosEjemplo = TestCase $ do
  ...
  where
    archivosDeEjemplo = do
      ...
      return [ "examples" </> nombre | nombre <- nombres, takeExtension nombre == ".txt" ]
...
-- Suite completa que se corre con cabal test.
tests :: Test
tests = TestList 
  [ TestLabel "Basicos originales" $ TestList [case1, case2, case3, case4, case5, case6, case7, case8, case9, case10, case11]
  , TestLabel "Funcion sqrt" testSqrt
  , TestLabel "Numeros negativos literales" testNegativosLiterales
  , TestLabel "Potencias con bases negativas" testNegativePowers
  , TestLabel "Identidades trigonometricas" testTrigIdentities
  ...
  , TestLabel "Validacion de dominios" testValidacionDominios
